...
public class SpencerGameMenu extends FXGLMenu{
    public SpencerGameMenu(MenuType type){
        ...
        mmButton.setTranslateY(FXGL.getAppHeight() / 2 - 40 / 2);
    ...
    protected Node createBackground(double width, double height){
        return new ImageView(FXGL.image("GameMenu.gif"));
    }
    @Override
    ...
    private static class GameMenuButton extends StackPane{
        public GameMenuButton(String name, Runnable action) {
            ...
            text.fillProperty().bind(Bindings.when(hoverProperty()).then(Color.BLACK).otherwise(Color.WHITESMOKE)
            );
            ...
            exitBg.fillProperty().bind(Bindings.when(hoverProperty()).then(Color.WHITE).otherwise(Color.BLACK)
            );
            exitText.fillProperty().bind(Bindings.when(hoverProperty()).then(Color.BLACK).otherwise(Color.WHITESMOKE)
            );
            ...
            hsBg.fillProperty().bind(Bindings.when(hoverProperty()).then(Color.WHITE).otherwise(Color.BLACK)
            );
            hsText.fillProperty().bind(Bindings.when(hoverProperty()).then(Color.BLACK).otherwise(Color.WHITESMOKE)
            );
        }
    }
}
